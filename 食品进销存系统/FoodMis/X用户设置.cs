...
namespace FoodMis
{
    public partial class X用户设置 : UserControl
    {
        ...
        private void X用户设置_Load(object sender, EventArgs e)
        {
            groupBox_P.Visible = false;
            String tel = "";
            conn.Open();
            string sql1 = "select * from salesperson where number='" + Username + "'";
        ...
        private void button_saveP_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    try
                    {
                        string sql = "update salesperson set password='" + pass2 + "' where number=" + Username;
                        conn.Open();
        ...
        private void button_save_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    string sql = "update salesperson set salesperson_name='" + NAme + "',phone='" + tell + "' where number=" + Username;
                    conn.Open();
                    command = new SqlCommand(sql, conn);
                    command.ExecuteNonQuery();
